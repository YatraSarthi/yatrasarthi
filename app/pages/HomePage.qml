...
Page {
    ...
    // Guards against onTextChanged re-firing the suggestion fetch when
    // we programmatically set the TextField's text after a selection.
    // Without this, selecting a suggestion sets .text, which triggers
    // onTextChanged, which restarts the search timer, which reopens
    // the dropdown — the exact "click twice" bug.
                    ...
                    delegate: Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                ...
                                // The suppress flag must be set
                                // immediately before .text is assigned —
                                // onTextChanged fires synchronously and
                                // checks this flag first, before it would
                                // otherwise restart the search timer.
                                suppressPickupFetch = true
